...
#            self.assertTrue("Progress of child coroutines accumulated to" in lst_alert_msgs[0])
    ...
    def test_memory(self):
        ...
        def get_memory():
            util.do_GC()
            return DotNetSystem.GC.GetTotalMemory(True)/1e6 if DotNetSystem else gc.get_count()[0] #@UndefinedVariable (never tested the "else" part...)
        before = get_memory()
        for _ in xrange(100):
            ao.foo().get()
        after = get_memory()
        assertions.fail_if(after-before > 10,'Memory difference is too large.before=%s after=%s' % (before,after))
